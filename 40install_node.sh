...
echo "checking node..." >> /var/log/cfn-init.log
OUT=$( [ ! -d "/opt/elasticbeanstalk/node-install" ] && echo "trying to install node.js $NODE_VER"  >> /var/log/cfn-init.log && mkdir /opt/elasticbeanstalk/node-install ; cd /opt/elasticbeanstalk/node-install/ && \
...
echo "YAY! Updated global NPM version to `npm -v`" >> /var/log/cfn-init.log
  ...
  echo "Skipping NPM -g version update. To update, please uncomment 40install_node.sh:12" >> /var/log/cfn-init.log
fi
